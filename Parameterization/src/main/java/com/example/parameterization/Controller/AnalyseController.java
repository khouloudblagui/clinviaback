...
import com.example.parameterization.dto.BioAnalysesDTO;
import com.example.parameterization.dto.BioAnalysesMapper;
...
@RestController
@CrossOrigin(origins = "*")
@RequestMapping("api/v1/parameterization/bioanalyses")
public class AnalyseController {

    private final BioAnalysesService bioAnalysesService;
    @Autowired
    private BioAnalysesMapper bioAnalysesMapper;
    @Autowired
    ...
    @PostMapping("/add")
    public BioAnalyses addBioAnalyses(@RequestBody BioAnalysesDTO bioAnalysesDTO) {
        System.out.println(bioAnalysesDTO.getBiologicalAnalysisRefValueMax());
        BioAnalyses bioAnalyses = bioAnalysesMapper.toEntity(bioAnalysesDTO);
        return bioAnalysesService.addBioAnalyses(bioAnalyses);
    }
